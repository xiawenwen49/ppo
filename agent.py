...
class Agent:
    def __init__(self,
        ...
        self._act, self._train = build_train(
            model=model, # mlp network for continuous action space, cnn network for discrete
        ...
        self.rollouts = [Rollout() for _ in range(nenvs)] # Rollout() gives a rollout instance
        self.t = 0

    def act(self, obs_t, reward_t, done_t, training=True): # training not used?
        # change state shape to WHC
        obs_t = list(map(self.phi, obs_t)) # change each element(state) in obs_t(states of n envs)
        ...
        if self.state_tm1['obs'] is not None: # add former timestep's info to rollout
            for i in range(self.nenvs):
                self.rollouts[i].add(
                    obs_t=self.state_tm1['obs'][i],
                    reward_tp1=reward_t[i], # from obs_t-1 to obs_t, then get rewatd_t
        ...
        # train network
        if training:
            if self.t > 0 and (self.t / self.nenvs) % self.time_horizon == 0: # time_horizon is 128, TD(128) 
                bootstrap_values = value_t.copy() # V(s) estimation
                self.train(bootstrap_values)  # training uses the horizon segment, and will clean the segment
        else: 
            if self.t > 0 and (self.t / self.nenvs) % self.time_horizon == 0:
                for rollout in self.rollouts: # flush self.rollouts, to avoid memory depletion
                    rollout.flush()
        ...
        self.state_tm1['obs'] = obs_t # state_tm1: one timestep's state info of n envs
    ...
    def train(self, bootstrap_values):
        ...
        obs_t = trajectories['obs_t'] # 2D (nenv, observation_length)
        ...
        # compute returns
        returns_t = compute_returns( # rewards_tp1 is used to compute returns_t
        ...
        if not self.use_lstm:
            ...
            obs_t = obs_t[:, indices] # change columns
        ...
        for epoch in range(self.epoch):
            for i in range(int(self.time_horizon / self.batch_size)):
                ...
                if self.continuous:
                    ...
                    batch_actions = self._pick_batch(actions_t, i) # all actions (of n envs)
                    batch_log_probs = self._pick_batch(log_probs_t, i)
                batch_obs = self._pick_batch(obs_t, i, shape=self.state_shape) # all states (of n envs)
                ...
                # training
                loss = self._train( # !!!.  st, at, return (for value func estimator), At, log pi_old(st, at), 
                    batch_obs, batch_actions, batch_returns, batch_advs,
                    batch_log_probs, batch_features, batch_masks)

        # clean trajectories
        for rollout in self.rollouts:
            rollout.flush() # after training, clean the horizon rollouts
        return loss

    def _rollout_trajectories(self): # n env's rollouts to one trajectory
        obs_t = [] # 2D (nenv, observation_length)
        actions_t = [] # 2D
        rewards_tp1 = []
        values_t = []
        log_probs_t = []
        ...
        masks_t = []
        for rollout in self.rollouts:
            obs_t.append(rollout.obs_t) # n env's obs_t
    ...
    def _pick_batch(self, data, batch_index, flat=True, shape=None):
        ...
        if flat:
            if shape is not None: 
                return np.reshape(batch_data, [-1] + shape) # continuous action and observation, need this shape [-1]+shape
