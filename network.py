...
def cnn_network(convs,
    ...
    out = make_fcs(fcs, out) # both policy and value use cnn layer and this fully-connected layer
    rnn_out, rnn_state = make_lstm(
        lstm_unit, nenvs, step_size, out, masks, rnn_state)

    if use_lstm: # if use_lstm is true
        out = rnn_out
    # out is the transformed observation

    policy = layers.fully_connected( # policy's fully-connected layer
    ...
    value = layers.fully_connected( # value's fully-connected layer
...
def mlp_network(fcs,
    ...
    with tf.variable_scope('policy'):
        out = make_fcs( # fully-connected layer transform of inpt, like features
        ...
        dist = tf.distributions.Normal(loc=policy, scale=std) # normal distribution
    ...
    with tf.variable_scope('value'):
        ...
        value = layers.fully_connected( # state value
...
def _make_network(convs,
                  ...
                  step_size,
                  scope):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE): # scope is the name string
        if continuous:
...
def make_network(convs, fcs, use_lstm=True, padding='VALID', continuous=False):
    ...
    # so this return is a function instance (lambda function), input args, output tf operations.
